...
class Expression
{
 public:
  ...
  // abstract type.
  static Expression*
  make_float(const llvm::APFloat&, Type*, Location);

  // Make a constant complex expression.  TYPE should be NULL for an
  // abstract type.
  static Expression*
  make_complex(const std::pair<llvm::APFloat, llvm::APFloat>&, Type*, Location);
...
class Numeric_constant
{
 public:
  Numeric_constant()
    : classification_(NC_INVALID), u_{llvm::APInt(0, (uint64_t)0)}, type_(NULL)
  { }
  ...
  void
  set_int(Type*, const llvm::APInt&);

  // Set to a rune value.
  void
  set_rune(Type*, const llvm::APInt&);

  // Set to a floating point value.
  void
  set_float(Type*, const llvm::APFloat&);

  // Set to a complex value.
  void
  set_complex(Type*, const std::pair<llvm::APFloat, llvm::APFloat>&);
  ...
  void
  get_int(llvm::APInt*) const;

  void
  get_rune(llvm::APInt*) const;

  void
  get_float(llvm::APFloat*) const;

  void
  get_complex(std::pair<llvm::APFloat, llvm::APFloat>*) const;
 ...
 private:
  ...
  To_unsigned_long
  mpz_to_unsigned_long(const llvm::APInt& ival, unsigned long *val) const;

  To_unsigned_long
  mpfr_to_unsigned_long(const llvm::APFloat& fval, unsigned long *val) const;
  ...
  union u__
  {
    ...
    // If NC_COMPLEX.
    std::pair<llvm::APFloat, llvm::APFloat> complex_val;
      ~u__() {}
  } u_;
