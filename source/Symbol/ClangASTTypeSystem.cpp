...
bool
ClangASTTypeSystem::IsAggregateType (void* type) const
{
...
bool
ClangASTTypeSystem::IsArrayType (void* type,
...
bool
ClangASTTypeSystem::IsVectorType (void* type,
...
bool
ClangASTTypeSystem::IsRuntimeGeneratedType (void* type) const
{
    clang::DeclContext* decl_ctx = ClangASTContext::GetASTContext(m_ast)->GetDeclContextForType(GetQualType(type));
    if (!decl_ctx)
        return false;
...
bool
ClangASTTypeSystem::IsCharType (void* type) const
{
...
bool
ClangASTTypeSystem::IsCompleteType (void* type) const
{
    const bool allow_completion = false;
    return GetCompleteQualType (m_ast, GetQualType(type), allow_completion);
}

bool
ClangASTTypeSystem::IsConst(void* type) const
{
    return GetQualType(type).isConstQualified();
}

bool
ClangASTTypeSystem::IsCStringType (void* type, uint32_t &length) const
{
...
bool
ClangASTTypeSystem::IsFunctionType (void* type, bool *is_variadic_ptr) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
uint32_t
ClangASTTypeSystem::IsHomogeneousAggregate (void* type, ClangASTType* base_type_ptr) const
{
    if (!type)
        return 0;
...
size_t
ClangASTTypeSystem::GetNumberOfFunctionArguments (void* type) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetFunctionArgumentAtIndex (void* type, const size_t index) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsFunctionPointerType (void* type) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsIntegerType (void* type, bool &is_signed) const
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::IsPointerType (void* type, ClangASTType *pointee_type) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsPointerOrReferenceType (void* type, ClangASTType *pointee_type) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsReferenceType (void* type, ClangASTType *pointee_type, bool* is_rvalue) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsFloatingPointType (void* type, uint32_t &count, bool &is_complex) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsDefined(void* type) const
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::IsObjCClassType (void* type) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsObjCObjectOrInterfaceType (void* type) const
{
    if (type)
...
bool
ClangASTTypeSystem::IsPolymorphicClass (void* type) const
{
    if (type)
    {
        clang::QualType qual_type(GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsPossibleDynamicType (void* type, ClangASTType *dynamic_pointee_type,
            ...
            case clang::Type::Builtin:
                if (check_objc && llvm::cast<clang::BuiltinType>(qual_type)->getKind() == clang::BuiltinType::ObjCId)
                {
                    if (dynamic_pointee_type)
                        dynamic_pointee_type->SetClangType(m_ast, type);
                    return true;
                }
                break;
...
bool
ClangASTTypeSystem::IsScalarType (void* type) const
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::IsTypedefType (void* type) const
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::IsVoidType (void* type) const
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::GetCXXClassName (void* type, std::string &class_name) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsCXXClassType (void* type) const
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::IsBeingDefined (void* type) const
{
    if (!type)
        return false;
    clang::QualType qual_type (GetCanonicalQualType(type));
...
bool
ClangASTTypeSystem::IsObjCObjectPointerType (void* type, ClangASTType *class_type_ptr)
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::GetObjCClassName (void* type, std::string &class_name)
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::GetCompleteType (void* type) const
{
    if (!type)
        return false;
...
//----------------------------------------------------------------------
size_t
ClangASTTypeSystem::GetPointerByteSize () const
{
    if (m_ast)
...
ConstString
ClangASTTypeSystem::GetTypeName (void* type) const
{
...
uint32_t
ClangASTTypeSystem::GetTypeInfo (void* type, ClangASTType *pointee_or_element_clang_type) const
{
    if (!type)
        return 0;
...
lldb::LanguageType
ClangASTTypeSystem::GetMinimumLanguage (void* type)
{
    if (!type)
...
lldb::TypeClass
ClangASTTypeSystem::GetTypeClass (void* type) const
{
    if (!type)
...
unsigned
ClangASTTypeSystem::GetTypeQualifiers(void* type) const
{
    if (type)
...
ClangASTType
ClangASTTypeSystem::AddConstModifier (void* type) const
{
    if (type)
    {
...
ClangASTType
ClangASTTypeSystem::AddRestrictModifier (void* type) const
{
    if (type)
    {
        clang::QualType result(GetQualType(type));
...
ClangASTType
ClangASTTypeSystem::AddVolatileModifier (void* type) const
{
    if (type)
    {
        clang::QualType result(GetQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetArrayElementType (void* type, uint64_t *stride) const
{
    if (type)
    {
        clang::QualType qual_type(GetCanonicalQualType(type));
        ...
        ClangASTType element_type (m_ast, array_eletype->getCanonicalTypeUnqualified());
...
ClangASTType
ClangASTTypeSystem::GetCanonicalType (void* type) const
{
    if (type)
        return ClangASTType (m_ast, GetCanonicalQualType(type));
    return ClangASTType();
}
...
ClangASTType
ClangASTTypeSystem::GetFullyUnqualifiedType (void* type) const
{
    if (type)
        return ClangASTType(m_ast, GetFullyUnqualifiedType_Impl(m_ast, GetQualType(type)));
    return ClangASTType();
}


int
ClangASTTypeSystem::GetFunctionArgumentCount (void* type) const
{
    if (type)
    {
...
ClangASTType
ClangASTTypeSystem::GetFunctionArgumentTypeAtIndex (void* type, size_t idx) const
{
    if (type)
    {
        const clang::FunctionProtoType* func = llvm::dyn_cast<clang::FunctionProtoType>(GetCanonicalQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetFunctionReturnType (void* type) const
{
    if (type)
    {
        clang::QualType qual_type(GetCanonicalQualType(type));
...
size_t
ClangASTTypeSystem::GetNumMemberFunctions (void* type) const
{
...
ClangASTTypeSystem::GetMemberFunctionAtIndex (void* type, size_t idx)
{
    std::string name("");
                    ...
                    if (cxx_record_decl)
                    {
                        ...
                        if (idx < static_cast<size_t>(std::distance(method_iter, method_end)))
                        {
                            ...
                            if (method_decl)
                            {
                                ...
                                clang_type = ClangASTType(m_ast,method_decl->getType().getAsOpaquePtr());
                            }
                        }
                    }
                }
...
ClangASTType
ClangASTTypeSystem::GetLValueReferenceType (void* type) const
{
    if (type)
    {
        return ClangASTType(m_ast, m_ast->getLValueReferenceType(GetQualType(type)));
    }
    return ClangASTType();
}

ClangASTType
ClangASTTypeSystem::GetRValueReferenceType (void* type) const
{
    if (type)
    {
        return ClangASTType(m_ast, m_ast->getRValueReferenceType(GetQualType(type)));
    }
    return ClangASTType();
}

ClangASTType
ClangASTTypeSystem::GetNonReferenceType (void* type) const
{
    if (type)
        return ClangASTType(m_ast, GetQualType(type).getNonReferenceType());
    return ClangASTType();
}

ClangASTType
ClangASTTypeSystem::CreateTypedefType (void* type, const char *typedef_name,
...
ClangASTType
ClangASTTypeSystem::GetPointeeType (void* type) const
{
    if (type)
    {
        clang::QualType qual_type(GetQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetPointerType (void* type) const
{
    if (type)
    {
        clang::QualType qual_type (GetQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetTypedefedType (void* type) const
{
    if (type)
    {
...
ClangASTType
ClangASTTypeSystem::RemoveFastQualifiers (void* type) const
{
    if (type)
    {
        clang::QualType qual_type(GetQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetBasicTypeFromAST (void* type, lldb::BasicType basic_type) const
{
    if (type)
...
uint64_t
ClangASTTypeSystem::GetBitSize (void* type, ExecutionContextScope *exe_scope) const
{
    if (GetCompleteType (type))
    {
        clang::QualType qual_type(GetCanonicalQualType(type));
        switch (qual_type->getTypeClass())
        {
            ...
            case clang::Type::ObjCObject:
            {
                ...
                if (process)
                {
                    ...
                    {
                        uint64_t bit_size = 0;
                        if (objc_runtime->GetTypeBitSize(ClangASTType(m_ast, qual_type), bit_size))
                            return bit_size;
                    }
                }
                else
...
size_t
ClangASTTypeSystem::GetTypeBitAlign (void* type) const
{
    if (GetCompleteType(type))
        return m_ast->getTypeAlign(GetQualType(type));
    return 0;
}


lldb::Encoding
ClangASTTypeSystem::GetEncoding (void* type, uint64_t &count) const
{
    if (!type)
...
lldb::Format
ClangASTTypeSystem::GetFormat (void* type) const
{
    if (!type)
...
uint32_t
ClangASTTypeSystem::GetNumChildren (void* type, bool omit_empty_base_classes) const
{
    if (!type)
        return 0;
...
lldb::BasicType
ClangASTTypeSystem::GetBasicTypeEnumeration (void* type) const
{
    if (type)
    {
        clang::QualType qual_type(GetQualType(type));
...
uint32_t
ClangASTTypeSystem::GetNumDirectBaseClasses (void* type) const
{
    if (!type)
        return 0;
...
uint32_t
ClangASTTypeSystem::GetNumVirtualBaseClasses (void* type) const
{
    if (!type)
        return 0;
...
uint32_t
ClangASTTypeSystem::GetNumFields (void* type) const
{
    if (!type)
        return 0;
...
ClangASTType
ClangASTTypeSystem::GetDirectBaseClassAtIndex (void* type, size_t idx, uint32_t *bit_offset_ptr) const
{
    if (!type)
        return ClangASTType();
...
ClangASTType
ClangASTTypeSystem::GetVirtualBaseClassAtIndex (void* type, size_t idx, uint32_t *bit_offset_ptr) const
{
    if (!type)
        return ClangASTType();
...
ClangASTType
ClangASTTypeSystem::GetFieldAtIndex (void* type, size_t idx,
                               std::string& name,
                               uint64_t *bit_offset_ptr,
                               uint32_t *bitfield_bit_size_ptr,
...
uint32_t
ClangASTTypeSystem::GetNumPointeeChildren (void* type) const
{
    if (!type)
        return 0;
...
ClangASTType
ClangASTTypeSystem::GetChildClangTypeAtIndex (void* type, ExecutionContext *exe_ctx,
                                        size_t idx,
...
size_t
ClangASTTypeSystem::GetIndexOfChildMemberWithName (void* type, const char *name,
                                             bool omit_empty_base_classes,
...
uint32_t
ClangASTTypeSystem::GetIndexOfChildWithName (void* type, const char *name, bool omit_empty_base_classes) const
{
    if (type && name && name[0])
    {
        clang::QualType qual_type(GetCanonicalQualType(type));
...
size_t
ClangASTTypeSystem::GetNumTemplateArguments (void* type) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
ClangASTType
ClangASTTypeSystem::GetTemplateArgument (void* type, size_t arg_idx, lldb::TemplateArgumentKind &kind) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
clang::EnumDecl *
ClangASTTypeSystem::GetAsEnumDecl (void* type) const
{
...
clang::RecordDecl *
ClangASTTypeSystem::GetAsRecordDecl (void* type) const
{
...
clang::CXXRecordDecl *
ClangASTTypeSystem::GetAsCXXRecordDecl (void* type) const
{
    return GetCanonicalQualType(type)->getAsCXXRecordDecl();
}

clang::ObjCInterfaceDecl *
ClangASTTypeSystem::GetAsObjCInterfaceDecl (void* type) const
{
...
clang::FieldDecl *
ClangASTTypeSystem::AddFieldToRecordType (void* type, const char *name,
                                    const ClangASTType &field_clang_type,
                                    AccessType access,
                                    uint32_t bitfield_bit_size)
{
    if (!type || !field_clang_type.IsValid())
        return nullptr;
...
void
ClangASTTypeSystem::BuildIndirectFields (void* type)
{
    clang::RecordDecl *record_decl = GetAsRecordDecl(type);
...
void
ClangASTTypeSystem::SetIsPacked (void* type)
{
    clang::RecordDecl *record_decl = GetAsRecordDecl(type);
...
clang::VarDecl *
ClangASTTypeSystem::AddVariableToRecordType (void* type, const char *name,
                                       const ClangASTType &var_type,
                                       AccessType access)
{
    clang::VarDecl *var_decl = nullptr;
    
    if (!type || !var_type.IsValid())
        return nullptr;
...
clang::CXXMethodDecl *
ClangASTTypeSystem::AddMethodToCXXRecordType (void* type, const char *name,
...
ClangASTTypeSystem::CreateBaseClassSpecifier (void* type, AccessType access, bool is_virtual, bool base_of_class)
{
    if (type)
        return new clang::CXXBaseSpecifier (clang::SourceRange(),
                                            ...
                                            m_ast->getTrivialTypeSourceInfo (GetQualType(type)),
                                            clang::SourceLocation());
    return nullptr;
}

void
ClangASTTypeSystem::DeleteBaseClassSpecifiers (void* type, clang::CXXBaseSpecifier **base_classes, unsigned num_base_classes)
{
...
bool
ClangASTTypeSystem::SetBaseClassesForClassType (void* type, clang::CXXBaseSpecifier const * const *base_classes,
...
bool
ClangASTTypeSystem::SetObjCSuperClass (void* type, const ClangASTType &superclass_clang_type)
{
    if (type && superclass_clang_type.IsValid() && superclass_clang_type.GetTypeSystem() == this)
    {
        clang::ObjCInterfaceDecl *class_interface_decl = GetAsObjCInterfaceDecl (type);
        clang::ObjCInterfaceDecl *super_interface_decl = GetAsObjCInterfaceDecl (superclass_clang_type.GetOpaqueQualType());
...
bool
ClangASTTypeSystem::AddObjCClassProperty (void* type, const char *property_name,
...
bool
ClangASTTypeSystem::IsObjCClassTypeAndHasIVars (void* type, bool check_superclass) const
{
    clang::ObjCInterfaceDecl *class_interface_decl = GetAsObjCInterfaceDecl (type);
    if (class_interface_decl)
...
ClangASTTypeSystem::AddMethodToObjCObjectType (void* type,
                                               ...
                                               bool is_artificial)
{
    ...
    clang::ObjCMethodDecl *objc_method_decl = clang::ObjCMethodDecl::Create (*m_ast,
                                                                             ...
                                                                             ClangASTContext::GetASTContext(m_ast)->GetDeclContextForType(GetQualType(type)),
                                                                             name[0] == '-',
                                                                             is_variadic,
                                                                             is_synthesized,
...
bool
ClangASTTypeSystem::SetHasExternalStorage (void* type, bool has_extern)
{
    if (!type)
        return false;
...
bool
ClangASTTypeSystem::StartTagDeclarationDefinition (void* type)
{
    if (type)
    {
        clang::QualType qual_type (GetQualType(type));
...
bool
ClangASTTypeSystem::CompleteTagDeclarationDefinition (void* type)
{
    if (type)
    {
        clang::QualType qual_type (GetQualType(type));
...
bool
ClangASTTypeSystem::AddEnumerationValueToEnumerationType (void* type, const ClangASTType &enumerator_clang_type,
...
ClangASTType
ClangASTTypeSystem::GetEnumerationIntegerType (void* type) const
{
    clang::QualType enum_qual_type (GetCanonicalQualType(type));
    const clang::Type *clang_type = enum_qual_type.getTypePtr();
    if (clang_type)
...
ClangASTType
ClangASTTypeSystem::CreateMemberPointerType (void* type, const ClangASTType &pointee_type) const
{
...
size_t
ClangASTTypeSystem::ConvertStringToFloatValue (void* type, const char *s, uint8_t *dst, size_t dst_size) const
{
    if (type)
    {
        clang::QualType qual_type (GetCanonicalQualType(type));
...
void
ClangASTTypeSystem::DumpValue (void* type, ExecutionContext *exe_ctx,
                         Stream *s,
                         lldb::Format format,
...
bool
ClangASTTypeSystem::DumpTypeValue (void* type, Stream *s,
                             lldb::Format format,
                             const lldb_private::DataExtractor &data,
...
void
ClangASTTypeSystem::DumpSummary (void* type, ExecutionContext *exe_ctx,
                           Stream *s,
                           const lldb_private::DataExtractor &data,
                           lldb::offset_t data_byte_offset,
                           size_t data_byte_size)
...
void
ClangASTTypeSystem::DumpTypeDescription (void* type) const
{
    StreamFile s (stdout, false);
    DumpTypeDescription (&s);
    ClangASTMetadata *metadata = ClangASTContext::GetMetadata (m_ast, type);
    if (metadata)
    {
        metadata->Dump (&s);
    }
...
void
ClangASTTypeSystem::DumpTypeDescription (void* type, Stream *s) const
{
    if (type)
    {
        clang::QualType qual_type(GetQualType(type));
...
bool
ClangASTTypeSystem::GetValueAsScalar (void* type, const lldb_private::DataExtractor &data,
                                lldb::offset_t data_byte_offset,
                                size_t data_byte_size,
                                Scalar &value) const
{
    ...
    {
        uint64_t count = 0;
        lldb::Encoding encoding = GetEncoding (type, count);

        if (encoding == lldb::eEncodingInvalid || count != 1)
            return false;

        const uint64_t byte_size = GetByteSize(type, nullptr);
        lldb::offset_t offset = data_byte_offset;
        switch (encoding)
        {
...
bool
ClangASTTypeSystem::SetValueFromScalar (void* type, const Scalar &value, Stream &strm)
{
    ...
    if (!IsAggregateType (type))
    {
        ...
        const uint64_t bit_width = GetBitSize(type, nullptr);
...
bool
ClangASTTypeSystem::ReadFromMemory (void* type, lldb_private::ExecutionContext *exe_ctx,
                              lldb::addr_t addr,
                              ...
                              lldb_private::DataExtractor &data)
{
    ...
    const uint64_t byte_size = GetByteSize(type, exe_ctx ? exe_ctx->GetBestExecutionContextScope() : NULL);
...
bool
ClangASTTypeSystem::WriteToMemory (void* type, lldb_private::ExecutionContext *exe_ctx,
                             lldb::addr_t addr,
                             AddressType address_type,
