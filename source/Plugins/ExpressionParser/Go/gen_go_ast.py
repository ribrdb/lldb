...
def addConstructor(name, parent, children):
    ...
    for x in childTypes:
        if x.startswith("[]"):
            filteredChildNames = []
            filteredChildTypes = []
            for i in xrange(len(childTypes)):
                if isValueType(childTypes[i]):
                    filteredChildNames.append(childNames[i])
                    filteredChildTypes.append(childTypes[i])
            childNames = filteredChildNames
